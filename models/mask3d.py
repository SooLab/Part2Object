...
class Mask3D(nn.Module):
    def __init__(
        ...
        if self.non_parametric_queries:
            ...
            self.query_feat_part = nn.Embedding(num_queries_part, hidden_dim)
            # learnable query p.e.
            self.query_pos_part = nn.Embedding(num_queries_part, hidden_dim)
        ...
        self.cross_attention_part = nn.ModuleList()
        self.self_attention_part = nn.ModuleList()
        self.ffn_attention_part = nn.ModuleList()
        self.lin_squeeze_part = nn.ModuleList()
        ...
        for _ in range(num_shared):
            ...
            self.cross_attention_part.append(tmp_cross_attention)
            self.self_attention_part.append(tmp_self_attention)
            self.ffn_attention_part.append(tmp_ffn_attention)
            # self.lin_squeeze_part.append(tmp_squeeze_attention)

        self.cross_attention = nn.ModuleList()
        self.cross_attention_p2o = nn.ModuleList()
        ...
        for _ in range(num_shared):
            tmp_cross_attention = nn.ModuleList()
            tmp_cross_attention_p2o = nn.ModuleList()
            ...
            for i, hlevel in enumerate(self.hlevels):
                ...
                tmp_cross_attention_p2o.append(
                    CrossAttentionLayer(
                        d_model=self.mask_dim,
                        nhead=self.num_heads,
                        dropout=self.dropout,
            ...
            self.cross_attention_p2o.append(tmp_cross_attention_p2o)
        ...
        self.score_head = nn.Sequential(nn.Linear(hidden_dim, hidden_dim), nn.ReLU(), nn.Linear(hidden_dim, 1))
    ...
    def forward(
        ...
        if self.non_parametric_queries:
            ...
            sampled_coords_part = torch.stack(
                [
                    coordinates.decomposed_features[i][fps_idx_part[i].long(), :]
                    for i in range(len(fps_idx_part))
                ]
            )
            ...
            query_pos_part = self.pos_enc(
                sampled_coords_part.float(), input_range=[mins, maxs]
            )  # Batch, Dim, queries
            query_pos_part = self.query_projection(query_pos_part)



            if not self.use_np_features:
                queries = torch.zeros_like(query_pos).permute((0, 2, 1))
                queries_part = torch.zeros_like(query_pos_part).permute((0, 2, 1))
                ...
                queries_part = self.np_feature_projection(queries_part)

            query_pos = query_pos.permute((2, 0, 1))
            query_pos_part = query_pos_part.permute((2, 0, 1))
            ...
            queries_part = self.query_feat_part.weight.unsqueeze(0).repeat(
                batch_size, 1, 1
            )
            query_pos_part = self.query_pos_part.weight.unsqueeze(1).repeat(
                1, batch_size, 1
            )
        ...
        predictions_mask_part = []
        predictions_score_part = []
        ...
        for decoder_counter in range(self.num_decoders):
            ...
            for i, hlevel in enumerate(self.hlevels):
                if self.train_on_segments:
                    output_class, outputs_mask, attn_mask2 = self.mask_module(
                        queries,
                        mask_features,
                        mask_segments,
                        len(aux) - hlevel - 1,
                    ...
                    output_class_part, outputs_mask_part, attn_mask = self.mask_module(
                        queries_part,
                        mask_features,
                        mask_segments,
                        len(aux) - hlevel - 1,
                ...
                decomposed_attn2 = attn_mask2.decomposed_features
                ...
                batched_attn2 = torch.stack(
                    [
                        decomposed_attn2[k][rand_idx[k], :]
                        for k in range(len(rand_idx))
                    ]
                )
                ...
                batched_attn2.permute((0, 2, 1))[
                    batched_attn2.sum(1) == rand_idx[0].shape[0]
                ] = False

                m = torch.stack(mask_idx)
                batched_attn = torch.logical_or(batched_attn, m[..., None])
                batched_attn2 = torch.logical_or(batched_attn2, m[..., None])
                ...
                output = self.cross_attention_part[decoder_counter][i](
                    queries_part.permute((1, 0, 2)),
                    src_pcd,
                    ...
                    query_pos=query_pos_part,
                )

                output = self.self_attention_part[decoder_counter][i](
                    output,
                    tgt_mask=None,
                ...
                # FFN
                queries_part = self.ffn_attention_part[decoder_counter][i](
                    output
                ).permute((1, 0, 2))

                output_score_part = self.score_head(queries_part)


                output = self.cross_attention_p2o[decoder_counter][i](
                ...
                output = self.cross_attention[decoder_counter][i](
                    output,
                    src_pcd,
                    memory_mask=batched_attn2.repeat_interleave(self.num_heads, dim=0).permute((0, 2, 1)),
                ...
                output_score = self.score_head(queries)


                predictions_class_part.append(output_class_part)
                predictions_mask_part.append(outputs_mask_part)
                predictions_score_part.append(output_score_part)
        ...
        if self.train_on_segments:
            output_class_part, outputs_mask_part = self.mask_module(
                queries_part,
                mask_features,
                mask_segments,
                0,
        ...
        return {
            ...
            "aux_outputs": self._set_aux_loss(
                predictions_class, predictions_mask, predictions_score
            ),
            "sampled_coords": sampled_coords.detach().cpu().numpy()
            if sampled_coords is not None
            else None,
            "backbone_features": pcd_features,
            "pred_part": {"pred_logits": predictions_class_part[-1], "pred_masks": predictions_mask_part[-1], "pred_scores": predictions_score_part[-1],}, 
            "part_results": self._set_aux_loss(
                predictions_class_part, predictions_mask_part, predictions_score_part
            ),
        }
    ...
    @torch.jit.unused
    def _set_aux_loss(self, outputs_class, outputs_seg_masks, outputs_scores):
        # this is a workaround to make torchscript happy, as torchscript
        # doesn't support dictionary with non-homogeneous values, such
        # as a dict having both a Tensor and a list.
        return [
            {"pred_logits": a, "pred_masks": b, "pred_scores": c}
            for a, b, c in zip(outputs_class[:-1], outputs_seg_masks[:-1], outputs_scores[:-1])
        ]
