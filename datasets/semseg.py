...
class SemanticSegmentationDataset(Dataset):
        ...
        for database_path in self.data_dir:
            ...
            if self.dataset_name != "s3dis":
                ...
                if not (database_path / f"{mode_s3dis}_database.yaml").exists():
                    print(f'b path {os.path.abspath(database_path)}')
    ...
    def splitPointCloud(self, cloud, size=50.0, stride=50, inner_core=-1):
        if inner_core == -1:
            ...
            conds_inner = []
            for (x, y) in cells:
                xcond_outer = (cloud[:, 0] <= x + inner_core / 2. + size / 2) & (
                        cloud[:, 0] >= x + inner_core / 2. - size / 2)
                ycond_outer = (cloud[:, 1] <= y + inner_core / 2. + size / 2) & (
                        cloud[:, 1] >= y + inner_core / 2. - size / 2)
    ...
    def __getitem__(self, idx: int):
        ...
        labels = np.vstack((points[:, 10], layer_masks)).T
        ...
        if self.dataset_name == "stpls3d":
            ...
            return coordinates, features, labels, self.data[idx]['instance_gt_filepath'].split("/")[-1][:-4], \
                   raw_color, raw_normals, raw_coordinates, idx
