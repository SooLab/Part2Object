...
def voxelize(batch, ignore_label, voxel_size, probing, mode, task,
    ...
    (coordinates_, features, labels, original_labels, inverse_maps, original_colors, original_normals,
    ...
    for sample in batch:
        ...
        coordinates_.append(torch.from_numpy(sample_coordinates).int())
    ...
    input_dict = {"coords": coordinates_, "feats": features}
    if len(labels) > 0:
        ...
        if len(list_labels[0].shape) == 1:
            ...
            if mode == "test":
                ...
                target = get_instance_masks(list_labels,
                                        ...
                                        scene_names = [sample[3] for sample in batch],
                                        coords=coordinates_)
                for i in range(len(target)):
                    target[i]["point2segment"] = input_dict["labels"][i][:, -1]
                if "train" not in mode:
                    ...
                    for i in range(len(target_full)):
                        target_full[i]["point2segment"] = torch.from_numpy(original_labels[i][:, -1]).long()
    else:
        target = []
        target_full = []
        coordinates = []
...
def get_instance_masks(
    ...
    for batch_id in range(len(list_labels)):
        ...
        for layer in range(1, 3):
            instance_ids = list_labels[batch_id][:, layer].unique()
            ...
            for instance_id in instance_ids:
                ...
                tmp = list_labels[batch_id][
                    list_labels[batch_id][:, layer] == instance_id
                ]
                # if layer == 5:
                label_id = tmp[0, 0]
                ...
                label_ids[layer-1].append(label_id)
                    
                masks[layer-1].append(list_labels[batch_id][:, layer] == instance_id)
                ...
                if list_segments:
                    ...
                    segment_masks[layer-1].append(segment_mask)

        if len(label_ids) == 0:
            return list()
        
        for layer4 in range(2):
            label_ids[layer4] = torch.stack(label_ids[layer4])
            label_ids[layer4] = torch.clamp(label_ids[layer4] - label_offset, min=0)
            masks[layer4] = torch.stack(masks[layer4])
        if list_segments:
            for layer3 in range(2):
                segment_masks[layer3] = torch.stack(segment_masks[layer3])
        ...
        if task == "semantic_segmentation":
            ...
            if list_segments:
                ...
                target.append(
                    {
                        "labels_layer1": label_ids[0],
                        "labels_layer2": label_ids[1],
                        
                        "masks_layer1": masks[0],
                        "masks_layer2": masks[1],
                        
                        "segment_mask_layer1": segment_masks[0],
                        "segment_mask_layer2": segment_masks[1],

                        # "include_matric": include_matric,
                        # "ratio_matric": ratio_matric,
