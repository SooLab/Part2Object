...
trainval_file = [line.rstrip() for line in open(join(BASE_DIR, 'ScanNet_splits/scannet.txt'))]
test_file = [line.rstrip()[:12]+'.ply' for line in open(join(BASE_DIR, 'ScanNet_splits/scannet.txt'))]
...
parser.add_argument('--input_path', type=str, default='scannet_plys/', help='raw data path')
...
def construct_superpoints(path):
    ...
    out_labels = np.ones_like(coords[:, 0]).squeeze()
    #

    if not os.path.exists(args.sp_path):
        os.makedirs(args.sp_path)
    np.save(args.sp_path + '/' + path[47:-4] + '_superpoint.npy', out_sp_labels)
