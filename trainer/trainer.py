...
def get_evenly_distributed_colors(count: int) -> List[Tuple[np.uint8, np.uint8, np.uint8]]:
    ...
    return list(map(lambda x: np.array(colorsys.hsv_to_rgb(*x)), HSV_tuples))
...
class SaveCheckpointingFrequency(pl.Callback):
    def on_train_epoch_end(
        self, trainer: "pl.Trainer", pl_module: "pl.LightningModule"
    ):

        if trainer.local_rank == 0 and trainer.current_epoch >= 50 and trainer.current_epoch % 50 == 0:
            general = pl_module.config.general
            # trainer.save_checkpoint(f"{general.save_dir}/epoch_{self.current_epoch}.ckpt")
            os.rename(f"{general.save_dir}/last-epoch.ckpt",f"{general.save_dir}/epoch_{trainer.current_epoch}.ckpt")
            print(f"{trainer.current_epoch} Checkpoint created")
...
class InstanceSegmentation(pl.LightningModule):
    def __init__(self, config):
        ...
        weight_dict = {"loss_ce": matcher.cost_class,
                       ...
                       "loss_score": 0.0,
                       "loss_ce_part": matcher.cost_class,
                       "loss_mask_part": matcher.cost_mask,
                       "loss_dice_part": matcher.cost_dice,
                       "loss_score_part": 0.0,
                       }
        ...
        # scores_weight_dict = {}
        # for j in range(1, 5):
        #     scores_weight_dict[f"score_loss_{j}"] = 10
        # weight_dict.update(scores_weight_dict)
        
        aux_weight_dict = {}
        for i in range(self.model.num_levels * self.model.num_decoders):
            if i not in self.config.general.ignore_mask_idx:
                aux_weight_dict.update({k + f"_{i}_part": v for k, v in weight_dict.items()})
            else:
                aux_weight_dict.update({k + f"_{i}_part": 0. for k, v in weight_dict.items()})
        weight_dict.update(aux_weight_dict)
    ...
    def training_step(self, batch, batch_idx):
        ...
        try:
            ...
            # output["pred_logits"] = torch.zeros_like(output["pred_logits"])
            # output["pred_logits"][:, :, 1] = 1
        ...
        try:
            losses, ind_ = self.criterion(output, target, mask_type=self.mask_type, epoch=self.current_epoch, data=data)
    ...
    def export(self, pred_masks, scores, pred_classes, file_names, decoder_id):
        ...
        pred_mask = np.zeros(pred_masks.shape[0])
        label = 1
        # pred_masks = np.flip(pred_masks, axis=1)
        for j in range(pred_masks.shape[1]):
            if scores[j] >= self.config.general.export_threshold:
                # print(pred_masks[:, j].astype(np.int64).sum())
                # exit()
                pred_mask[pred_masks[:, j].astype(np.int64) == 1] = label
                label += 1
    ...
    def save_visualizations(self, target_full, full_res_coords,
        ...
        if 'labels' in target_full:
            ...
            for instance_counter, (label, mask) in enumerate(zip(target_full['labels'], target_full['masks'])):
                ...
                gt_inst_pcd_color.append(instances_colors[instance_counter % len(instances_colors)].unsqueeze(0).repeat(
        ...
        for did in range(len(sorted_masks)):
            ...
            if len(pred_coords) > 0:
                ...
                torch.save(pred_inst_color, f"vis_layer200_2/{file_name}_{self.decoder_id}_mask.pth")
                torch.save(pred_coords, f"vis_layer200_2/{file_name}_{self.decoder_id}_coords.pth")
    ...
    def eval_step(self, batch, batch_idx):
        ...
        # file_names = self.all_gather(file_names)
        ...
        try:
            ...
            # self.scores = output['pred_scores']
            # print()
            # output["pred_logits"] = torch.zeros_like(output["pred_logits"])
            # output["pred_logits"][:, :, 1] = 1
        ...
        if self.config.data.test_mode != "test":
            ...
            try:
                losses, ind = self.criterion(output, target,
                                        mask_type=self.mask_type, epoch=self.current_epoch, data=data)
                ...
                # if losses['loss_mask'] + losses["loss_dice"] < 
            except ValueError as val_err:
                print(f"ValueError: {val_err}")
                print(f"data shape: {data.shape}")
                print(f"data feat shape:  {data.features.shape}")
        ...
        # self.config.general.export = False
        if self.config.data.test_mode != "test":
    ...
    def get_mask_and_scores(self, mask_cls, mask_pred, num_queries=100, num_classes=18, device=None, output=None):
        if device is None:
            device = self.device
        labels = torch.arange(num_classes, device=device).unsqueeze(0).repeat(num_queries, 1).flatten(0, 1)

        if self.config.general.topk_per_image != -1:
            scores_per_query, topk_indices = mask_cls.flatten(0, 1).topk(self.config.general.topk_per_image,
                                                                         sorted=False)
        else:
            scores_per_query, topk_indices = mask_cls.flatten(0, 1).topk(num_queries, sorted=False)
                ...
                for instance_id in range(norm_overlaps.shape[0]):
                    ...
                    if not (sort_scores_values[instance_id] < self.config.general.scores_threshold):
                        ...
                        if not sorted_masks[:, instance_id].sum() == 0.0:
                            overlap_ids = set(
                                np.nonzero(norm_overlaps[instance_id, :] > self.config.general.iou_threshold)[0])
                ...
                if 'cond_inner' in self.test_dataset.data[idx[bid]]:
                    ...
                    self.save_visualizations(target_full_res[bid],
                                             ...
                                             query_pos=all_query_pos[bid][
                                                 self.test_dataset.data[idx[bid]]['cond_inner']] if len(
    ...
    def eval_instance_epoch_end(self):
        ...
        root_path = f"eval_output"
        base_path = f"{root_path}/instance_evaluation_{self.config.general.experiment_name}_{self.current_epoch}_{self.local_rank}/"
        ...
        if self.validation_dataset.dataset_name in ["scannet", "stpls3d", "scannet200"]:
            ...
            # gt_data_path = f"/inspurfs/group/yangsb/yangbin/data/processed/scannetpp/instance_gt/validation"

            # gt_data_path = f"{self.validation_dataset.data_dir[0]}/instance_gt/trival"
            # gt_data_path = "/inspurfs/group/yangsb/yangbin/instance_gt"

        else:
            gt_data_path = f"{self.validation_dataset.data_dir[0]}/instance_gt/Area_{self.config.general.area}"

        pred_path = f"{base_path}/tmp_output_{self.local_rank}.txt"
    ...
    def val_dataloader(self):
        ...
        sampler = DistributedSampler(self.validation_dataset)
        return hydra.utils.instantiate(
